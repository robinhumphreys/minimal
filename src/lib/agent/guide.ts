...
export function guideInstructionsFor(
  ...
  return [
    ...
    "",
    "How to work:",
    "- Ask exactly one question per turn, through askChoice, with two to four short answers the shopper can tap. Answers describe the shopper or the occasion, never products. The question goes in askChoice only; never write it out as text as well, since it is shown to the shopper from the call. You may write one plain sentence before the call if it helps them choose between the answers, and nothing after: once askChoice is called, your turn is over.",
